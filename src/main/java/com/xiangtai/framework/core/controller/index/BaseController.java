package com.xiangtai.framework.core.controller.index;
...
import com.xiangtai.framework.core.util.FormMap;
...
public class BaseController {
	 ...
	 * @return
	 */
	public UserFormMap getSecurityInfo(){
		HttpServletRequest request = ((ServletRequestAttributes)RequestContextHolder.getRequestAttributes()).getRequest();
		// 通过工具类获取当前登录的bean
		return (UserFormMap)Common.findUserSession(request);
	}
		...
		try {
			...
			while (en.hasMoreElements()) {
				...
				if(nms.endsWith("[]")){
					...
					if(as!=null&&as.length!=0&& !Objects.equals(Arrays.toString(as), "[]")){
    ...
    /**
     *
     * 根据userId,resKey 获取数据权限
     * @param resKey
     * @return
     */
    public String getdataAuthorityByResKey(String resKey) {
		int userId = getSecurityUserId();
		HashMap<String,Object> resFormMap = new HashMap<>();
		resFormMap.put("userId",userId);
		resFormMap.put("resKey",resKey);
		return resourcesMapper.getdataAuthorityByResKey(resFormMap);
    }
}